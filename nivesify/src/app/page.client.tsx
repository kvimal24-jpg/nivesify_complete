...
/* ── TYPES + SHARED LOGIC (server derives the same shape) ───────────────── */
import { type LiveFund, type Manifest, type InsightRow, type LandingData, num, hasNum, shortName, buildLandingData } from "@/lib/landing-shared";
...
function Counter({ to, suffix = "", duration = 1700 }: { to: number; suffix?: string; duration?: number }) {
  /* SSR + first paint render the REAL number (SEO/no-CLS). The count-up
     replays only when the stat sits below the fold at hydration. */
...
/* ══════════════════════════════════════════════════════════════════════════
   LIVE DATA — server-rendered via props; client fetch only as fallback
   (when the RSC load fails, e.g. local dev without network).
══════════════════════════════════════════════════════════════════════════ */
function useFallbackData(enabled: boolean) {
  const [state, setState] = useState<LandingData | null>(null);
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      if (alive) setState(buildLandingData(funds, manifest, insights));
    });
    return () => { alive = false; };
  }, [enabled]);
  ...
  return (
    <div className="glass" style={{ borderRadius: 28, padding: "clamp(20px,3vw,28px)" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 18, gap: 10 }}>
        <div>
          <div style={{ fontSize: 10.5, fontWeight: 800, color: "var(--txt-lo)", letterSpacing: ".14em", textTransform: "uppercase" as const, marginBottom: 6 }}>Playground · go ahead, drag</div>
      ...
      <div style={{ marginBottom: 14 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline" as const, marginBottom: 8 }}>
          <span style={{ fontSize: 12.5, fontWeight: 700, color: "var(--txt-md)" }}>Monthly investment</span>
      ...
      <div style={{ marginBottom: 18 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline" as const, marginBottom: 8 }}>
          <span style={{ fontSize: 12.5, fontWeight: 700, color: "var(--txt-md)" }}>Stay invested for</span>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 7, marginBottom: 10 }}>
        {([
          ...
          <div key={i} style={{ borderRadius: 12, padding: "9px 11px", background: bg, border: "1px solid rgba(255,255,255,.08)" }}>
            <div style={{ fontSize: 10, fontWeight: 800, letterSpacing: ".06em", color: "var(--txt-lo)", textTransform: "uppercase" as const, marginBottom: 3 }}>{l}</div>
            <div style={{ fontSize: 14, fontWeight: 900, color: c, fontFamily: "Fraunces,Georgia,serif" }}>{v}</div>
          </div>
        ))}
      </div>
      <p style={{ fontSize: 10.5, color: "rgba(210,220,245,.30)", fontStyle: "italic", textAlign: "center" as const }}>Assumed returns for illustration — not guarantees</p>
      ...
      <div style={{ borderTop: "1px solid rgba(255,255,255,.09)", marginTop: 16, paddingTop: 16 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 5 }}>
          ...
          <span style={{ fontSize: 12, fontWeight: 800, color: "var(--mint)", letterSpacing: ".1em", textTransform: "uppercase" as const }}>{hz.tag}</span>
        </div>
        <p style={{ fontSize: 12.5, color: "var(--txt-md)", lineHeight: 1.65, marginBottom: 12 }}>{hz.why}</p>
          ...
          return (
            <div key={f.Fund_Name} className="glass-chip lift" style={{ borderRadius: 14, padding: "10px 12px", marginBottom: 7, display: "flex", alignItems: "center", gap: 10 }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 12.5, fontWeight: 700, color: "rgba(255,255,255,.90)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" as const }}>{shortName(f.Fund_Name)}</div>
                <div style={{ fontSize: 10.5, fontWeight: 600, color: "var(--txt-lo)" }}>{f.Sub_Category}</div>
              </div>
              <div style={{ textAlign: "right" as const, flexShrink: 0 }}>
                <div style={{ fontSize: 13, fontWeight: 900, color: "var(--mint)" }}>{num(f.Fund_Return_3Y).toFixed(1)}%</div>
                <div style={{ fontSize: 10, fontWeight: 700, color: "var(--txt-lo)" }}>3Y CAGR</div>
              </div>
            </div>
          );
        })}
        {loaded && picks.length === 0 && <div style={{ fontSize: 12, color: "var(--txt-lo)", padding: "6px 2px" }}>Live rankings refreshing…</div>}
  ...
  return (
    <section style={{ position: "relative", padding: "clamp(48px,7vw,90px) clamp(16px,4vw,48px)" }}>
      <div style={{ maxWidth: 1240, margin: "0 auto" }}>
        ...
        <SR>
          <div style={{ textAlign: "center" as const, marginBottom: 26 }}>
            <div style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "rgba(99,102,241,.12)", border: "1px solid rgba(99,102,241,.30)", borderRadius: 100, padding: "6px 16px", marginBottom: 16 }}>
              ...
              <span style={{ fontSize: 11.5, fontWeight: 800, color: loaded ? "#A5B4FC" : "var(--txt-lo)", letterSpacing: ".12em", textTransform: "uppercase" as const }}>
              ...
              {!loaded || tickerFunds.length === 0 ? <div style={{ padding: "0 24px" }}><Skel h={14} /></div> : (
                <div className="marquee-track">
                  {[...tickerFunds, ...tickerFunds].map((f, i) => (
                    <div key={i} style={{ display: "flex", alignItems: "center", gap: 8, padding: "0 18px", whiteSpace: "nowrap" as const, borderRight: "1px solid rgba(255,255,255,.05)", height: 26 }}>
                      <span style={{ fontSize: 12, fontWeight: 700, color: "var(--txt-md)" }}>{f.name}</span>
                      <span style={{ fontSize: 11, fontWeight: 800, color: f.alpha >= 0 ? "#00C97B" : "#EF4444", background: f.alpha >= 0 ? "rgba(0,201,123,.10)" : "rgba(239,68,68,.10)", border: `1px solid ${f.alpha >= 0 ? "rgba(0,201,123,.25)" : "rgba(239,68,68,.25)"}`, borderRadius: 100, padding: "1px 7px" }}>
                ...
                {HOF_CATS.map(c => (
                  <button key={c.key} role="tab" aria-selected={tab === c.key} onClick={() => setTab(c.key)}
                    style={{
                      ...
                      borderRadius: 100, padding: "0 20px", fontSize: 13, fontWeight: 800,
                      ...
                      <div key={f.Fund_Name}
                        ...
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 10, marginBottom: 14 }}>
                          ...
                          <div style={{ textAlign: "right" as const, flexShrink: 0 }}>
                            ...
                            <div style={{ fontSize: 10, fontWeight: 800, color: "var(--txt-lo)", letterSpacing: ".09em" }}>3Y CAGR</div>
                          </div>
                        </div>

                        <div style={{ display: "flex", gap: 6, flexWrap: "wrap" as const }}>
                          <span style={{
                            fontSize: 11.5, fontWeight: 800, borderRadius: 100, padding: "4px 10px",
                          ...
                          <span style={{ fontSize: 11.5, fontWeight: 700, color: "var(--txt-md)", background: "rgba(255,255,255,.05)", border: "1px solid rgba(255,255,255,.10)", borderRadius: 100, padding: "4px 10px" }}>
                            AUM {fmtAUMcr(num(f.Current_AUM))}
                          </span>
                        </div>
                        <div style={{ marginTop: 12, fontSize: 11.5, fontWeight: 700, color: "#A5B4FC", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        {hasNum(avgRet) && (
          <SR delay={120}>
            <p style={{ textAlign: "center" as const, marginTop: 16, fontSize: 12.5, color: "var(--txt-lo)" }}>
          ...
          <div className="duo flip">
            ...
            <div>
              <div className="glass-deep float" style={{ borderRadius: 26, padding: "clamp(20px,3vw,30px)" }}>
                <div style={{ display: "inline-flex", alignItems: "center", gap: 7, background: "rgba(0,201,123,.12)", border: "1px solid rgba(0,201,123,.28)", borderRadius: 100, padding: "4px 13px", marginBottom: 16 }}>
                  <span style={{ fontSize: 10.5, fontWeight: 800, color: "#00C97B", letterSpacing: ".10em", textTransform: "uppercase" as const }}>Sample report</span>
                </div>
                  ...
                  {[
                    ...
                    <div key={i} className="glass-chip" style={{ borderRadius: 15, padding: "13px 15px", display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12 }}>
                      <div style={{ minWidth: 0 }}>
                        ...
                        <div style={{ fontSize: 11.5, color: "var(--txt-lo)", marginTop: 2 }}>XIRR {r.xirr} · {r.note}</div>
                      </div>
                      <span style={{ flexShrink: 0, fontSize: 11.5, fontWeight: 800, padding: "5px 12px", borderRadius: 100, color: r.c, background: `${r.c}1f`, border: `1px solid ${r.c}52` }}>{r.v}</span>
                    </div>
                  ))}
                </div>
                <p style={{ marginTop: 14, fontSize: 11.5, color: "var(--txt-lo)", fontStyle: "italic" }}>Illustrative example — your report uses your actual transactions</p>
              </div>
            </div>
            ...
            <div>
              ...
              {hasNum(industry?.Pct_Funds_Beating_Benchmark_3Y) && (
                <p style={{ marginTop: 14, fontSize: 12.5, color: "var(--txt-lo)" }}>
...
/* ══════════════════════════════════════════════════════════════════════════
   PAGE ASSEMBLY
══════════════════════════════════════════════════════════════════════════ */
export default function LandingClient({ server }: { server: LandingData | null }) {
  const fallback = useFallbackData(!server);
  const data = server ?? fallback;

  const manifestReportDate = data?.reportDate ?? "";
  const industry = data?.industry ?? null;
  const tickerFunds = data?.tickerFunds ?? [];
  const loaded = !!data;
  const topByCategory = (sub: string): LiveFund[] => data?.topByCategory[sub] ?? [];
  const totalSchemes = data?.totalSchemes ?? 0;
    ...
    <main style={{ background: "var(--ink)", minHeight: "100vh", fontFamily: "'DM Sans',system-ui,sans-serif", color: "var(--txt-hi)" }}>
        ...
        <div style={{ maxWidth: 1200, margin: "0 auto", position: "relative" }}>
          <div className="hero-cols">
            <div>
              <div style={{ marginBottom: 14 }}>
                <SleepDoodle />
                <p style={{ margin: "2px 0 0", fontSize: 11.5, color: "var(--txt-lo)", fontStyle: "italic", textAlign: "right" as const }}>you, after one honest portfolio review</p>
              </div>
              ...
              <div className="f1" style={{ display: "inline-flex", alignItems: "center", gap: 9, background: "rgba(0,201,123,.09)", border: "1px solid rgba(0,201,123,.26)", borderRadius: 100, padding: "6px 15px", marginBottom: 20 }}>
                ...
                <span style={{ fontSize: 12, fontWeight: 700, color: "#00E8A2", letterSpacing: ".07em", textTransform: "uppercase" as const }}>
                  {loaded ? `${totalSchemes.toLocaleString("en-IN")} funds · updated ${manifestReportDate} · free forever` : "connecting to live data…"}
                </span>
              </div>
              ...
              <div className="f5" style={{ display: "flex", gap: "20px 28px", flexWrap: "wrap" as const, paddingTop: 22, borderTop: "1px solid rgba(255,255,255,.08)" }}>
                {[
                  ...
                  <div key={i}>
                    ...
                    <div style={{ fontSize: 12, color: "var(--txt-lo)", fontWeight: 600, marginTop: 5 }}>{s.l}</div>
                  </div>
                ))}
                <div>
                  ...
                  <div style={{ fontSize: 12, color: "var(--txt-lo)", fontWeight: 600, marginTop: 5 }}>cost, ads &amp; commissions</div>
                </div>
              </div>
            </div>
      ...
      <section style={{ padding: "clamp(44px,6vw,76px) clamp(16px,4vw,48px)", borderTop: "1px solid rgba(255,255,255,.05)" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <SR>
            <p style={{ fontSize: 12, fontWeight: 800, color: "#00E8A2", letterSpacing: ".12em", textTransform: "uppercase" as const, textAlign: "center" as const, marginBottom: 8 }}>Start · Audit · Plan</p>
                ...
                <Link key={i} href={p.href} style={{ textDecoration: "none" }}>
                  <div className="glass-deep lift journey-card" style={{ height: "100%", display: "flex", flexDirection: "column" as const, position: "relative", overflow: "hidden" }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
                      ...
                      <span style={{ fontSize: 11, fontWeight: 800, letterSpacing: ".14em", textTransform: "uppercase" as const, color: p.ink, opacity: .75 }}>{p.step}</span>
                    </div>
        ...
        <div style={{ maxWidth: 1200, margin: "0 auto", position: "relative" }}>
          <SR>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", flexWrap: "wrap" as const, gap: 10, marginBottom: 22 }}>
              <div>
                <p style={{ fontSize: 12, fontWeight: 800, color: "#A5B4FC", letterSpacing: ".12em", textTransform: "uppercase" as const, marginBottom: 8 }}>The calm-money toolkit</p>
              ...
              {[
                ...
                <Link key={c.id} href={`/dashboard/calculators#${c.id}`} className="calc-tile" style={{ textDecoration: "none", display: "block", height: "100%" }}>
                  <div className="glass lift calc-card">
                    ...
                    <div style={{ fontSize: 12.5, color: "var(--txt-md)", lineHeight: 1.65, marginBottom: 12 }}>{c.d}</div>
                    <span style={{ fontSize: 12.5, fontWeight: 800, color: c.ink }}>Calculate
      ...
      <div className={`sticky-cta glass-deep ${showCta ? "show" : ""}`} aria-hidden={!showCta}>
        <div style={{ minWidth: 0 }}>
          ...
          <div style={{ fontSize: 11, color: "var(--txt-lo)", fontWeight: 600 }}>true XIRR · keep/exit calls</div>
        </div>
